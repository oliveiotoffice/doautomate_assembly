    ...
    return (
      <div
        ...
        style={{
          ...
          padding: 4,
          display: "grid",
          gridTemplateColumns: "repeat(4, minmax(0, 1fr)) minmax(92px, 0.58fr)",
      ...
      >
        {inspectionLayout.map((name, layoutIndex) => {
          ...
          const actual = actuals[i] ?? parseReq(p.required);
          const pass = checkPass(p, actual);
          if (name === "QR Grade") return <QRTile key={name} C={C} pass={pass} style={{ gridColumn: 5, gridRow: "1 / span 2" }} />;
          return (
            <MetricReadout
              key={p.name}
              param={p}
              ...
              C={C}
              style={{
                gridColumn: (layoutIndex % 4) + 1,
                gridRow: Math.floor(layoutIndex / 4) + 1,
              }}
            />
          );
        })}
    ...
    <div className="dashboard-shell assembly-dashboard" style={{ ...MONO, "--h-scale": heightScale, width: "100%", height: "100%", minHeight: 0, overflow: "hidden", background: C.bg, transition: "background 0.25s ease, color 0.25s ease", display: "flex", flexDirection: "column" } as CSSProperties}>
      ...
      <style>{`
        ...
        .inspection-grid{
          ...
          grid-template-columns:repeat(4,minmax(0,1fr)) minmax(118px,0.72fr) !important;
